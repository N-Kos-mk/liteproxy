...
- **ホーム画面に追加**：ホーム画面（`/`）をブラウザの「ホーム画面に追加」から追加すると、アプリのように開けます。追加したアプリは `/app`（アドレス欄・戻る/進む・画像/本文切替/元ページの各ボタンと、中身を表示するiframeだけの外枠）から起動します。外枠自体はService Workerでキャッシュされますが、中継先のページ（`/p`・`/v`など、iframeの中身）はキャッシュの対象にはなりません。iframeの中身にあるツールバーは非表示のまま、外枠のボタンから操作します。ブラウザから直接 `/` を開いた場合や、ブックマーク・検索結果からの直接アクセスはこれまでどおりの表示のままです。
...
| — | Webアプリ（PWA）化 第二段階（`/app` に外枠 + iframe化） | 完了 |
| — | Web Share Target（Android、他アプリの「共有」から開く） | 検討中 |
