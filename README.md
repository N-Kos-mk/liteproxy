...
- **ホーム画面に追加**：ホーム画面（`/`）をブラウザの「ホーム画面に追加」から追加すると、アプリのように開けます。追加したアプリは `/app`（アドレス欄・戻る/進むボタンと、中身を表示するiframeだけの外枠）から起動します。外枠自体はService Workerでキャッシュされますが、中継先のページ（`/p`・`/v`など、iframeの中身）はキャッシュの対象にはなりません。ブラウザから直接 `/` を開いた場合や、ブックマーク・検索結果からの直接アクセスはこれまでどおりの表示のままです。
